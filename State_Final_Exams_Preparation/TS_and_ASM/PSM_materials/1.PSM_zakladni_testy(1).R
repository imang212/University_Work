...
pt(T, length(vyska) - 1) + 1 - pt(-T, length(vyska) - 1); 2*pt(T, length(vyska) - 1)
# pst je symetricka kolem nuly
t.test(vyska, mu = 168) # kontrolni test
  # p-hodnota 0.09659 > alfa 0.05 -> nezamitame H0, neprokazalo se, ze by vyska matek nemohla byt rovna 168 cm
...
shapiro.test(hmot) # p-hodnota 0.1623 > alfa => nezamitam H0
...
shapiro.test(vek) # p-hodnota 0.00134 < alfa => zamitam H0
...
tapply(cislo, kategorie, shapiro.test)
  # obe p-hodnoty vetsi nez alfa -> nezamitame H0, data maji normalni rozdeleni -> pouziji t-test
# nejprve graficke zobrazeni
boxplot(cislo ~ kategorie, main="Porodni hmotnost podle pohlavi", col=c(2,4))n# vidime poradi kategorii, muze se hodit
# testujeme hypotezy
...
MeanDiffCI(cislo ~ kategorie)m# pocita pro variantu s ruznymi rozptyly
###############################
...
# Normalita se testuje pro kazdou skupinu zvlast
#   H0: data maji normalni rozdeleni vs. H1: data nemaji normalni rozdeleni
par(mfrow=c(1,2)); tapply(cislo,kategorie,PlotQQ); par(mfrow=c(1,1)) # body lezi na obloucich v obou pripadech
tapply(cislo, kategorie, shapiro.test) # obe p-hodnoty < alfa -> zamitame H0, data nemaji normalni rozdeleni
# Wilcoxonuv test
...
(tab <- table(vzdel,otec))
  prop.table(tab, 1) # absolutni a relativni cetnosti
